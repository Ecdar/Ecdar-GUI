...
public class SystemEdgePresentation extends Group implements SelectHelper.ItemSelectable {
    ...
    private void initializeBinding(final EcdarSystemEdge edge) {
        ...
        if (edge.isFinished()) {
            ...
            link.endXProperty().bind(EcdarController.getActiveCanvasShellPresentation().getController().canvasPresentation.mouseTracker.gridXProperty().subtract(Grid.GRID_SIZE / 2));
            link.endYProperty().bind(EcdarController.getActiveCanvasShellPresentation().getController().canvasPresentation.mouseTracker.gridYProperty().subtract(Grid.GRID_SIZE / 2));
        }
