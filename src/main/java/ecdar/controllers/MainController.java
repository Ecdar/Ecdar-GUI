...
import ecdar.test.MutationTestPlan;
...
public class MainController implements Initializable {
    ...
    private void initializeFileExportAsPng() {
        ...
        menuBarFileExportAsPng.setOnAction(event -> {
            ...
            final ComponentPresentation presentation = ecdarPresentation.getController().canvas.getController().getActiveComponentPresentation();
    ...
    private WritableImage takeSnapshot() {
        ...
        ecdarPresentation.getController().canvas.getController().disallowGrid();
        image = scaleAndTakeSnapshot();
        ecdarPresentation.getController().canvas.getController().allowGrid();

        return image;
    }

    /**
     * Scale a Component or System View 4 times to get a higher resolution screenshot.
     * After the snapshot has been taken, the scale is returned to 1 times the size
     * @return the snapshot
     */
    private WritableImage scaleAndTakeSnapshot() {
        final WritableImage image;
        ecdarPresentation.getController().canvas.setScaleX(4.0);
        ecdarPresentation.getController().canvas.setScaleY(4.0);
        image = ecdarPresentation.getController().canvas.snapshot(new SnapshotParameters(), null);
        ecdarPresentation.getController().canvas.setScaleX(1.0);
        ecdarPresentation.getController().canvas.setScaleY(1.0);
        return image;
    }
    ...
    private void updateMenuItems() {
        switch (currentMode.get()) {
            case Editor:
                ...
                KeyboardTracker.unregisterKeybind(KeyboardTracker.ZOOM_RESET);
                break;
            case Simulator:
    ...
    private void setExtraZoomKeybindings() {
        ...
        KeyboardTracker.registerKeybind(KeyboardTracker.ZOOM_RESET, new Keybind(zoomResetCombination, (keyEvent) -> {
            keyEvent.consume();
