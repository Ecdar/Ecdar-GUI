...
public class ProcessController extends ModelController implements Initializable {
     ...
     */
    public void highlightEdges(final SystemEdge[] edges) {
        for (int i = 0; i < edges.length; i++) {
            final SystemEdge edge = edges[i];

            // Note that SystemEdge contains a Location type from the UPPAAL libraries
            // but we use a different Location class in our Maps
            final com.uppaal.model.core2.Location source = edge.getEdge().getSource();
            String sourceName = "";
            final com.uppaal.model.core2.Location target = edge.getEdge().getTarget();
            String targetName = "";

            // Match the Locations of the SystemEdge with a SystemLocation of the process,
            // so we can get the source/target names (names are not available on a Location)
            for (SystemLocation sysloc : edge.getProcess().getLocations()) {
                if (sysloc.getLocation() == source) {
                    sourceName = sysloc.getName();
                } else if (sysloc.getLocation() == target) {
